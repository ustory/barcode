...
public class BarcodeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String format = determineFormat(request);//调用determineFormat方法确定要生成的条形码文件类型，如jpg,png...
            int orientation = 0;

            Configuration cfg = buildCfg(request);//调用buildCfg方法，读取生成条形码的参数，如长度，宽度等信息

            String msg = request.getParameter(BARCODE_MSG);//条形码内容
            if (msg == null) {
                msg = "0123456789";
            }
            ...
            response.setHeader("Content-Disposition","attachment;filename=barcode." + format.substring(format.indexOf("/")+1));    
